	...
	<script type="text/javascript">
		...
		$(document).ready(function() {
        	$('#edit_category').ajaxForm({
        		success: function(){
        			updateCategory(<xsl:value-of select="$categoria_id" />);
        		}
        	});
    });
	</script>
	<form name="editar_categoria_{$categoria_id}" id="edit_category" method="post" action="{$adminroot}?m=category&amp;action=BackEdit">
		...
		<li class="floatFix">
			<label for="description">Destacada en Home</label>
			<select name="category_highlight" id="category_highlight">
				<option value="0">No</option>
				<option value="1">
					<xsl:if test="content/category/@highlight=1">
		...
		<li class="botones">
			<button class="btn btn-inverse">Guardar</button>&#xa0;&#xa0;<button class="btn cancel" onclick="return false;">Cancelar</button>
		</li>
	</ul>
	</form>
</xsl:template>
