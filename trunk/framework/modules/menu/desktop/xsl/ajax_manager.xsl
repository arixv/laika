	...
	<ul class="form floatFix menus">
		...
		<li class="floatFix">
			...
			<select name="menu_state" id="menu_state">
				<option value="1"><xsl:if test="content/menu/@state=1" ><xsl:attribute name="selected" >selected</xsl:attribute></xsl:if>Activo</option>
				<option value="0"><xsl:if test="content/menu/@state=0" ><xsl:attribute name="selected" >selected</xsl:attribute></xsl:if>Inactivo</option>
			</select>
		</li>
		<li class="botones">
			<button class="btn btn-inverse">Save</button>&#xa0;&#xa0;<button class="btn cancel" onclick="return false;">Cancel</button>
		</li>
	</ul>
	</form>
</xsl:template>
...
<xsl:template name="menu.add_subMenu">
	...
	<form name="add_subMenu_{$menu_id}" action="{$adminroot}?m=menu&amp;action=BackAdd" method="POST">
		...
		<ul class="form floatFix">
			...
			<li class="botones">
				<button class="boton azul">Save</button>&#xa0;&#xa0;<button class="boton cancel" onclick="return false;">Cancel</button>
			</li>
		</ul>
	</form>
</xsl:template>
